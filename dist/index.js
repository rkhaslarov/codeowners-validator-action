...
exports.isRuleRelevantToFolders = exports.ruleMatchesPath = exports.isValidCodeOwnersLine = void 0;
const fs = __importStar(__nccwpck_require__(147));
const minimatch_1 = __nccwpck_require__(501);
 ...
 */
function isValidCodeOwnersLine(line) {
    return !line.startsWith('#') && line.trim().length > 0;
}
exports.isValidCodeOwnersLine = isValidCodeOwnersLine;
/**
 * Parses the CODEOWNERS file and returns a Map of rules with match counters (initialized to 0)
 */
function parseCodeOwnersFile(codeOwnersFilePath) {
    var _a, e_1, _b, _c;
...
function getAllFiles(directoryPath) {
    return __awaiter(this, void 0, void 0, function* () {
        const items = yield fsAsync.readdir(directoryPath, { withFileTypes: true });
        const files = [];
        const directories = [];
        ...
        const nestedFiles = yield Promise.all(directories.map(dir => getAllFiles(dir)));
 ...
 */
function scanAllFiles(rootFolders) {
    return __awaiter(this, void 0, void 0, function* () {
        const allFiles = yield Promise.all(rootFolders.map(folder => getAllFiles(folder)));
...
function ruleMatchesPath(rule, filePath) {
    ...
    if (hasGlob) {
        return (0, minimatch_1.minimatch)(cleanPath, cleanRule, { matchBase: true });
    }
    ...
    return ((0, minimatch_1.minimatch)(cleanPath, cleanRule) ||
        (0, minimatch_1.minimatch)(cleanPath, cleanRule + '/**'));
}
 ...
 */
function isRuleRelevantToFolders(rule, folders) {
    const cleanRule = normalizePath(rule);
    for (const folder of folders) {
        ...
        if (cleanRule.includes('*')) {
            ...
            if ((0, minimatch_1.minimatch)(cleanFolder, cleanRule, { matchBase: true })) {
                return true;
            }
...
exports.isRuleRelevantToFolders = isRuleRelevantToFolders;
/**
 * Checks if a file path is owned by any rule and increments the counter for every matching rule.
 */
function checkOwnership(filePath, rules) {
    let owned = false;
 ...
 */
function validateCodeOwners(codeOwnersFilePath, foldersToValidate) {
    return __awaiter(this, void 0, void 0, function* () {
        const [{ rules }, allFilePaths] = yield Promise.all([
        ...
        const uncoveredFiles = allFilePaths.filter(filePath => !checkOwnership(filePath, rules));
        // Find rules that are relevant to the folders but never matched any file (counter is 0)
        const unusedRules = [...rules.entries()]
            .filter(([rule, count]) => count === 0 && isRuleRelevantToFolders(rule, foldersToValidate))
            .map(([rule]) => rule);
        // Collect all error messages
        const errorMessages = [];
        if (unusedRules.length > 0) {
            errorMessages.push('CODEOWNERS contains rules that do not match any files in the specified folders:', ...unusedRules.map(rule => `  - ${rule}`), '', 'Please remove these entries or verify the paths exist.');
        }
        if (uncoveredFiles.length > 0) {
            errorMessages.push('The following files do not have owners:', ...uncoveredFiles.map(path => `  - ${path}`), '', 'Please add rules to CODEOWNERS to cover these files.');
        }
...
const balanced_match_1 = __nccwpck_require__(839);
...
const brace_expansion_1 = __nccwpck_require__(709);
